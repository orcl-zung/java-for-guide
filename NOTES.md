...
- 8/30 学员真实经历披露：嘉立创实际在 **SMT 部门**（上游 PCB 订单→SMT 订单，算法解析客户 BOM/坐标文件，工厂贴装生产）；倾向保留营销平台包装，教练主张写真实 SMT 故事（差异化/无编造/订单域主线），学员半同意先出 SMT 版。已完成 SMT 技术栈情报扫描（learning-records/0005：官方 JD SpringCloud+MySQL+Redis/Memcached/RabbitMQ/Kafka；**C++ 线负责 CAM 解析→简历用"调度对接"口径**；2026-06 面经：MySQL 最拉分+明确筛项目造假）；8 项锚点已发学员待回填，回填后重写 0003 嘉立创段 + reference/0002 嘉立创章节
